module.exports = async (params) => {
  ...
  const title = await quickAddApi.inputPrompt("Titel der Quiz-Quelle");
  if (!title) {
    return;
  }
  ...
  const folder = (await quickAddApi.inputPrompt("Zielordner im Vault", "Lernen/Quiz-Quellen")) || "Lernen/Quiz-Quellen";

  const safeTitle = title.replace(/[\\/:*?"<>|]/g, "-").trim();
  const filePath = await ensureUniquePath(app, `${folder}/${safeTitle}.md`);
  const content = buildQuizSourceNote(title, modulId, jahr, relevanz);
...
function buildQuizSourceNote(title, modulId, jahr, relevanz) {
