...
export function deriveModuleId(path, existing) {
    if (existing) {
        return existing;
    }
...
export function parseLearningNote(path, markdown) {
    ...
    return {
        ...
        lernstatus: typeof fm.lernstatus === "string" ? fm.lernstatus : undefined,
        lerntyp: typeof fm.lerntyp === "string" ? fm.lerntyp : undefined,
        modul_id: deriveModuleId(path, typeof fm.modul_id === "string" ? fm.modul_id : undefined),
        pruefungsrelevanz: typeof fm.pruefungsrelevanz === "string" ? fm.pruefungsrelevanz : undefined,
...
export function updateYamlField(markdown, key, value) {
    const lines = markdown.split(/\r?\n/);
    if (lines[0]?.trim() !== FRONTMATTER_DELIMITER) {
        return `---\n${key}: "${value}"\n---\n\n${markdown}`;
    }
