import { ExamAttemptAnswer, ExamAttemptResult, ExamDefinition, ExamQuestion } from "./types";
import { parseFrontmatter } from "./notes";

function parseQuestionBlock(block: string, index: number): ExamQuestion | null {
  ...
  const correctIndexes = options.reduce<number[]>((accumulator, line, optionIndex) => {
...
export function parseExamMarkdown(markdown: string): ExamDefinition {
  ...
  return {
    ...
    questions: sections.map((section, index) => parseQuestionBlock(section, index)).filter((item): item is ExamQuestion => Boolean(item))
  };
}

export function gradeAttempt(exam: ExamDefinition, answers: ExamAttemptAnswer[]): ExamAttemptResult {
  let score = 0;
